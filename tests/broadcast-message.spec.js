...
test.describe('同域', () => {
  ...
  const evaluateFunc = (bmOpts) => {
    return new Promise((resolve, reject) => {
      ...
      console.log(`[BroadcastMessage]`, broadcastMessage, bmOpts)

      broadcastMessage.addEventListener('message', (event) => {
        console.log(`[BroadcastMessage-Event][onMessage][End-to-End-Testing]`, event.data)
        resolve(event.data)
      })
...
test.describe('跨域', () => {
  ...
  const evaluateReceiveFunc = ({ bmOpts, postMessageCount }) => {
    return new Promise((resolve, reject) => {
      ...
      broadcastMessage.addEventListener('message', (event) => {
        ...
        // console.log(`[BroadcastMessage-Event][onMessage][End-to-End-Testing-cross-domain]`, data, msgArr)

        if (msgArr.length >= postMessageCount) {
          /* 计算接收到的数据和发送数据的顺序是否一致 */

          const tmpArr1 = msgArr.slice(0, postMessageCount / 2)
          const tmpArr2 = msgArr.slice(postMessageCount / 2, postMessageCount).reverse()
          let isTrue = true
  ...
  test('正常创建并发送消息', async ({ page }) => {
    ...
    const evaluateArgs = { bmOpts, postMessageCount }
    await page2.evaluate(evaluateTransmitFunc, evaluateArgs)
    const result = await page.evaluate(evaluateReceiveFunc, evaluateArgs)
